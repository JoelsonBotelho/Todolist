...
import com.botelho.todolist.utils.Utils;
...
public class TaskController {
    ...
    public ResponseEntity create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if (currentDate.isAfter(taskModel.getStartDat()) || currentDate.isAfter(taskModel.getEndDat())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("A data de inicio/termino deve ser maior que a data atual");
        }
        if (taskModel.getStartDat().isAfter(taskModel.getEndDat())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
    ...
    public ResponseEntity update(@RequestBody TaskModel taskModel, HttpServletRequest request, @PathVariable UUID id) {
        ...
        var taskUpadted = this.taskRepository.save(task);
        return ResponseEntity.ok().body(taskUpadted);
    }
}